...
class TestActionServer:
    async def _recv_until_key(self, ws, key: str, timeout_s: float = 2.0):
    ...
    async def test_separate_sessions(self, setup_test_client: TestClient):
        ...
        message_1 = await self._recv_until_key(web_socket_1, "loopback_payload")
        message_2 = await self._recv_until_key(web_socket_2, "loopback_payload")
    ...
    async def test_multiple_sockets_single_session(self, setup_test_client: TestClient):
        ...
        for web_socket in web_sockets:
            message = await self._recv_until_key(web_socket, "loopback_payload")
    ...
    async def test_execute_new_processes_mocked(self, setup_test_client: TestClient):
        ...
        assert response.status == 200
        response_dict = await self._recv_until_key(web_socket, "new_processes")
    ...
    async def test_execute_new_process(self, setup_test_client: TestClient):
        ...
        assert response.status == 200
        response_dict = await web_socket.receive_json()
    ...
    async def test_execute_sleep_and_interrupt(self, setup_test_client: TestClient):
        ...
        assert_dicts_json_equal(
            dict(
                ...
                processes={
                    pid1: dict(
                        login_message=response_dict.get("processes", {}).get(pid1, {}).get("login_message"),
                        new_commands=[
