...
class Process(Base):
    ...
    created_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    session: Mapped['Session'] = relationship(back_populates='processes')
    commands: Mapped[list['Command']] = relationship(back_populates='process', cascade='all, delete-orphan')
...
class Command(Base):
    ...
    created_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    process: Mapped['Process'] = relationship(back_populates='commands')
    inputs: Mapped[list['Input']] = relationship(back_populates='command', cascade='all, delete-orphan')
    outputs: Mapped[list['Output']] = relationship(back_populates='command', cascade='all, delete-orphan')

class Input(Base):
    __tablename__ = 'inputs'
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    command_id: Mapped[str] = mapped_column(ForeignKey('commands.id', ondelete='CASCADE'))
    input_text: Mapped[str] = mapped_column(Text)
    created_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    command: Mapped['Command'] = relationship(back_populates='inputs')
